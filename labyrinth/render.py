...
from .art import FRONTISPIECE_MARK, TOGGLE_EMBLEM
from .markup import BodyContext, HTMLAccumulator, Heading, render_markdown_paragraphs, render_start_tag
...
TOC_LEADER_DOTS = ".&nbsp;" * 64
HTML_NAV_SEPARATOR = '<span class="site-utility-separator" aria-hidden="true">/</span>'
XHTML_NAV_SEPARATOR = '<xhtml:span class="site-utility-separator" aria-hidden="true">/</xhtml:span>'
FEED_ABSOLUTE_ATTRS = frozenset({"href", "poster", "src"})
SCRIPT_SOURCE_DIR = Path(__file__).resolve().with_name("script_sources")
...
@cache
def read_script_source(name: str) -> str:
    return (SCRIPT_SOURCE_DIR / name).read_text(encoding="utf-8").strip()


def render_script_block(name: str) -> str:
    return join_html_lines(
        "<script>",
        read_script_source(name),
        "</script>",
    )
...
def html_output_path(public_path: str) -> Path:
    if public_path == HOME_PUBLIC_PATH:
        return Path("index.html")
    return Path(public_path.lstrip("/")) / "index.html"


def work_asset_output_path(work: WorkDocument, relative_path: Path) -> Path:
    return Path(work.public_path.lstrip("/")) / relative_path
...
def render_pages(graph: SiteGraph) -> list[RenderedPage]:
    ...
    rendered = [
        RenderedPage(
            ...
            output_path=html_output_path(HOME_PUBLIC_PATH),
    ...
    if write_item is not None:
        rendered.append(
            RenderedPage(
                ...
                output_path=html_output_path(WRITE_PUBLIC_PATH),
    ...
    for work in graph.works:
        rendered.append(
            RenderedPage(
                ...
                output_path=html_output_path(work.public_path),
                html=render_work_page(graph, work),
                source_path=work.body_path,
                assets=tuple(
                    (asset.source_path, work_asset_output_path(work, asset.relative_path))
...
def render_home_page(graph: SiteGraph) -> str:
    ...
    content = join_html_lines(
        ...
        indent_html(render_home_frontispiece(graph, urls=urls), 4),
        "  </section>",
...
def render_write_page(graph: SiteGraph, item: LinkItem) -> str:
    ...
    content = join_html_lines(
        '<div class="page utility-page write-page" id="write">',
        '  <div class="page-body utility-body write-body">',
        indent_html(render_tally_embed(form_src), 4),
        indent_html(render_site_utility_nav(graph, urls=urls, modifier_class="write-utility-nav"), 4),
        "  </div>",
        "</div>",
    )
    return render_page(
...
def render_tally_embed(form_src: str) -> str:
    return join_html_lines(
        ...
        f"<script>{read_script_source('tally-widget-loader.js')}</script>",
    )
...
def render_page(
    *,
    ...
    return join_html_lines(
        ...
        indent_html(render_script_block("theme-boot.js"), 2),
        ...
        "  </label>",
        indent_html(render_script_block("theme-control.js"), 2),
...
def render_sidebar_primary(graph: SiteGraph, current_work: WorkDocument | None, urls: PageUrls) -> str:
    ...
    return join_html_lines(
        ...
        "  </div>",
        "</section>",
        render_site_utility_nav(graph, urls=urls, modifier_class="site-rail-utility-nav"),
    )
...
def render_home_frontispiece(graph: SiteGraph, *, urls: PageUrls) -> str:
    epigraph_html = render_home_epigraph(graph)
    read_href = escape(urls.relative_href(HOME_PUBLIC_PATH, fragment="contents"))
    cue_html = join_html_lines(
        f'<a class="frontispiece-cue" href="{read_href}" aria-label="Skip to contents">',
        '  <span class="frontispiece-cue-mark" aria-hidden="true">&darr;</span>',
        "</a>",
    )
    return join_html_lines(
        '<div class="frontispiece">',
        '  <span class="frontispiece-mark-wrap" aria-hidden="true">',
        indent_html(FRONTISPIECE_MARK, 4),
        "  </span>",
        indent_html(epigraph_html, 2),
        "</div>",
        indent_html(render_site_utility_nav(graph, urls=urls, modifier_class="home-utility-nav"), 0),
        cue_html,
    )


def render_home_epigraph(graph: SiteGraph) -> str:
    ...
    return join_html_lines(
        '<div class="frontispiece-epigraph">',
        *(
            f'  <p class="frontispiece-epigraph-line">{paragraph.html}</p>'
...
def render_site_utility_nav(graph: SiteGraph, *, urls: PageUrls, modifier_class: str) -> str:
    class_name = f"site-nav site-utility-nav {modifier_class}"
    items = tuple(render_utility_link(item, urls=urls) for item in ordered_site_header_links(graph.site.home.links))
    return join_html_lines(
        f'<nav class="{class_name}" aria-label="Site links">',
        indent_html(render_separated_items(items, HTML_NAV_SEPARATOR), 2),
        "</nav>",
    )


def render_utility_link(item: LinkItem, *, urls: PageUrls) -> str:
    classes = ["site-link", "site-utility-link"]
...
def render_separated_items(items: tuple[str, ...], separator_html: str) -> str:
    parts: list[str] = []
...
def render_mobile_work_end_matter(graph: SiteGraph, work: WorkDocument, *, urls: PageUrls) -> str:
    ...
    return join_html_lines(
        ...
        f"    {date_html}",
        "  </div>",
        indent_html(render_site_utility_nav(graph, urls=urls, modifier_class="mobile-work-utility-nav"), 2),
...
def render_mobile_work_section_links(graph: SiteGraph, work: WorkDocument, *, urls: PageUrls) -> str:
    ...
    items = join_html_lines(
        *(
            render_works_entry(section_work, urls=urls)
...
def render_feed_guide(graph: SiteGraph, *, feed_url: str) -> str:
    ...
    return join_html_lines(
        ...
        *paragraph_html,
        indent_html(render_feed_utility_nav(graph), 2),
        "</xhtml:section>",
    )
...
def render_feed_utility_nav(graph: SiteGraph) -> str:
    feed_urls = page_urls(graph, FEED_PUBLIC_PATH)
    links = [
    ...
    for label, href in links:
        parts.append(f'<xhtml:a class="site-link site-utility-link" href="{xml_escape(href)}">{xml_escape(label)}</xhtml:a>')
    return join_html_lines(
        '<xhtml:nav class="site-nav site-utility-nav feed-utility-nav" aria-label="Site links">',
        indent_html(
            render_separated_items(tuple(parts), XHTML_NAV_SEPARATOR),
            2,
        ),
        "</xhtml:nav>",
    )
    ...
    for page in rendered_pages:
        if '<link rel="canonical"' not in page.html:
            raise BuildError(
                ...
                f"{page.public_path} is missing a canonical link",
            )
...
class FeedContentRewriter(HTMLAccumulator):
    def __init__(self, urls: PageUrls) -> None:
        super().__init__()
        self.urls = urls

    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        self.append_html(render_start_tag(tag, self.rewrite_attrs(attrs), self_closing=False))

    def handle_startendtag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        self.append_html(render_start_tag(tag, self.rewrite_attrs(attrs), self_closing=True))

    def rewrite_attrs(self, attrs: list[tuple[str, str | None]]) -> list[tuple[str, str | None]]:
        rewritten: list[tuple[str, str | None]] = []
        for name, value in attrs:
            if value is None or name not in FEED_ABSOLUTE_ATTRS:
